...
namespace iguana::clas12::rga {
  ...
  static inline void SaveAndDisposeCanvas(TCanvas* c, char const* path_png)
  {
    if(!c)
      return;
    c->Modified();
  ...
  void FiducialFilterPass2Validator::BookIfNeeded()
  {
    ...
    auto mk = [](char const* name, char const* title) {
  ...
  bool FiducialFilterPass2Validator::Run(hipo::banklist& banks) const
  {
    ...
    // track torus polarity stats (labels for DC summary)
    {
      ...
      if(e_out)
        const_cast<FiducialFilterPass2Validator*>(this)->m_torus_out_events++;
      else
        const_cast<FiducialFilterPass2Validator*>(this)->m_torus_in_events++;
    }
    ...
    for(auto const& row : particle.getRowList()) {
      ...
      int pid  = particle.getInt("pid", row);
      if(pid == 11 || pid == 22)
        eorg_after.insert(pidx);
      if(pid == 211 || pid == 321 || pid == 2212 || pid == -211 || pid == -321 || pid == -2212)
        had_after.insert(pidx);
      if(pid > 0)
        pos_after.insert(pidx);
      else if(pid < 0)
        neg_after.insert(pidx);
    }
    ...
    if(m_have_calor) {
      ...
      for(int i = 0; i < n; ++i) {
        if(cal.getInt("layer", i) != 1)
          continue; // PCal only
        int pidx = cal.getInt("pindex", i);
        if(!eorg_before.count(pidx))
          continue;
        ...
        bool const survived = eorg_after.count(pidx);
        ...
        if(pid == 11) {
          ...
          if(survived)
            af_e[sector].insert(pidx);
        }
        else {
          be_g[sector].insert(pidx);
          if(survived)
            af_g[sector].insert(pidx);
        }
      }
      ...
      for(int i = 0; i < n; ++i) {
        ...
        if(pid == 11) {
          if(!seen_b_e.count(pidx)) {
            HH.before->Fill(x, y);
            seen_b_e.insert(pidx);
          }
          if(eorg_after.count(pidx) && !seen_a_e.count(pidx)) {
            HH.after->Fill(x, y);
            seen_a_e.insert(pidx);
          }
        }
        else {
          if(!seen_b_g.count(pidx)) {
            HH.before->Fill(x, y);
            seen_b_g.insert(pidx);
          }
          if(eorg_after.count(pidx) && !seen_a_g.count(pidx)) {
            HH.after->Fill(x, y);
            seen_a_g.insert(pidx);
          }
        }
      }
    ...
    if(m_have_traj) {
      ...
      for(int i = 0; i < n; ++i) {
        if(traj.getInt("detector", i) != 5)
          continue; // CVT
        if(traj.getInt("layer", i) != 12)
          continue;

        int pidx = traj.getInt("pindex", i);
        if(!had_before.count(pidx))
          continue;
        ...
        double phi           = std::atan2(y, x) * (180.0 / kPI);
        if(phi < 0)
          phi += 360.0;
        double rho   = std::sqrt(x * x + y * y);
        double theta = std::atan2(rho, (z == 0.0 ? 1e-9 : z)) * (180.0 / kPI);

        if(!b_seen.count(pidx)) {
    ...
    if(m_have_traj) {
      ...
      for(int i = 0; i < n; ++i) {
        ...
        if(!is_pos_before && !is_neg_before)
          continue;
      ...
      for(auto& kv : pos_r1) {
        if(m_dc_pos.r1_before)
          m_dc_pos.r1_before->Fill(kv.second);
        if(pos_after.count(kv.first) && m_dc_pos.r1_after)
          m_dc_pos.r1_after->Fill(kv.second);
      }
      for(auto& kv : pos_r2) {
        if(m_dc_pos.r2_before)
          m_dc_pos.r2_before->Fill(kv.second);
        if(pos_after.count(kv.first) && m_dc_pos.r2_after)
          m_dc_pos.r2_after->Fill(kv.second);
      }
      for(auto& kv : pos_r3) {
        if(m_dc_pos.r3_before)
          m_dc_pos.r3_before->Fill(kv.second);
        if(pos_after.count(kv.first) && m_dc_pos.r3_after)
          m_dc_pos.r3_after->Fill(kv.second);
      }
      for(auto& kv : neg_r1) {
        if(m_dc_neg.r1_before)
          m_dc_neg.r1_before->Fill(kv.second);
        if(neg_after.count(kv.first) && m_dc_neg.r1_after)
          m_dc_neg.r1_after->Fill(kv.second);
      }
      for(auto& kv : neg_r2) {
        if(m_dc_neg.r2_before)
          m_dc_neg.r2_before->Fill(kv.second);
        if(neg_after.count(kv.first) && m_dc_neg.r2_after)
          m_dc_neg.r2_after->Fill(kv.second);
      }
      for(auto& kv : neg_r3) {
        if(m_dc_neg.r3_before)
          m_dc_neg.r3_before->Fill(kv.second);
        if(neg_after.count(kv.first) && m_dc_neg.r3_after)
          m_dc_neg.r3_after->Fill(kv.second);
      }

      auto set_from_keys = [](std::unordered_map<int, double> const& m) {
      std::set<int> s; for (auto& kv : m) s.insert(kv.first); return s; };

      auto inter3 = [](std::set<int> const& A,
                       std::set<int> const& B, std::set<int> const& C) -> size_t {
        std::set<int> const* smallest = &A;
        ...
        size_t cnt = 0;
        for(int v : *smallest)
          if(A.count(v) && B.count(v) && C.count(v))
            ++cnt;
        return cnt;
      };

      auto keep_if_survived = [](std::set<int> const& S, std::unordered_set<int> const& survivors) {
  ...
  // plotting
  void FiducialFilterPass2Validator::DrawCalCanvas(int pid, char const* title)
  {
    ...
    for(int s = 1; s <= 6; ++s) {
      ...
      H.lv_before->SetLineColor(kBlue + 1);
      H.lv_before->SetLineWidth(2);
      H.lw_before->SetLineColor(kRed + 1);
      H.lw_before->SetLineWidth(2);
      H.lv_after->SetLineColor(kBlue + 1);
      H.lv_after->SetLineWidth(2);
      H.lv_after->SetLineStyle(2);
      H.lw_after->SetLineColor(kRed + 1);
      H.lw_after->SetLineWidth(2);
      H.lw_after->SetLineStyle(2);
  ...
  void FiducialFilterPass2Validator::DrawFTCanvas2x2()
  {
    ...
    auto draw_pad = [&](int pad, TH2F* h, char const* ttl) {
      c->cd(pad);
      gPad->SetLeftMargin(0.12);
      gPad->SetRightMargin(0.04);
      gPad->SetBottomMargin(0.12);
  ...
  void FiducialFilterPass2Validator::DrawCVTCanvas1x2(char const* title)
  {
    if(!m_have_traj || !m_cvt_before || !m_cvt_after)
      return;

    auto* c = new TCanvas("rgafid_cvt_l12_all", title, 1200, 600);
    c->Divide(2, 1);

    double const left   = 0.12;
    double const right  = 0.16;
    double const bottom = 0.12;
    double const top    = 0.08;

    double pct = 0.0;
    if(m_cvt_before_n > 0)
      pct = 100.0 * double(m_cvt_after_n) / double(m_cvt_before_n);
  ...
  void FiducialFilterPass2Validator::DrawDCCanvas2x3(DCHists const& H,
                                                     char const* bend, double survive_pct)
  {
  ...
  void FiducialFilterPass2Validator::Stop()
  {
    ...
    char const* pos_bend_id = electron_out ? "inb" : "out";
    char const* neg_bend_id = electron_out ? "out" : "inb";
    ...
    if(m_out) {
      ...
      // DC
      auto write_dc = [&](DCHists const& H) {
        write_obj(H.r1_before);
        write_obj(H.r1_after);
        write_obj(H.r2_before);
        write_obj(H.r2_after);
        write_obj(H.r3_before);
        write_obj(H.r3_after);
      };
      write_dc(m_dc_pos);
      write_dc(m_dc_neg);
