...
| `IGUANA_PREFIX`                                          | the path to the Iguana installation prefix, equivalent to `pkg-config iguana --variable prefix`; this is a convenience for consumers that do not use `pkg-config` or the other standard environment variables, however usage of this variable is **discouraged** since installation subdirectory names are dependent on build options |
